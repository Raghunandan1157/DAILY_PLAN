    ...
    buffer.innerHTML = `
        <div class="chart-card grid-full">
            ...
            <div style="padding: 16px; border-top: 1px solid var(--border-color); display:flex; justify-content:flex-end; gap:12px;">
                ...
                <button class="btn btn-outline" onclick="downloadCurrentStatusReport()" style="padding:10px 24px; border-color: var(--primary); color: var(--primary);">
                    <svg class="icon" viewBox="0 0 24 24" style="stroke: var(--primary);"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
                    Download Current Status Report
                </button>
                <button class="btn btn-outline" onclick="downloadStatusComparisonPNG()" style="padding:10px 24px; border-color: #7C3AED; color: #7C3AED;">
                    <svg class="icon" viewBox="0 0 24 24" style="stroke: #7C3AED;"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
...
// --- DOWNLOAD CURRENT STATUS REPORT ---
async function downloadCurrentStatusReport() {
    const fromDate = state.dateFrom;
    const toDate = state.dateTo;
    ...
    const label = formatRangeLabel(fromDate, toDate);
    setLoading(true, `Generating Current Status Report for ${label}...`);
    ...
    try {
        ...
        const cHeader = '#000000';
        const cWhite = '#FFFFFF';
        const cSuccess = '#D1FAE5';
        const cWarning = '#FEF3C7';
        const cDanger = '#FEE2E2';
        ...
        <head>
            ...
            <style>
                table { border-collapse: collapse; width: 100%; }
                th { border: 1px solid #000; text-align: center; font-family: Arial, sans-serif; font-size: 10pt; background: #000; color: #fff; font-weight: bold; }
                td { border: 1px solid #000; text-align: left; font-family: Arial, sans-serif; font-size: 10pt; white-space: nowrap; padding: 4px; }
                .txt-right { text-align: right; }
                .txt-center { text-align: center; }
                .title-row { text-align: center; font-size: 14pt; font-weight: bold; background: #FFFFFF; color: #000000; height: 30px; border: none; }
                .status-completed { background: ${cSuccess}; color: #065F46; font-weight: 600; }
                .status-pending-achieve { background: ${cWarning}; color: #92400E; font-weight: 600; }
                .status-pending-plan { background: #E0F2FE; color: #075985; font-weight: 600; }
                .status-no-data { background: ${cDanger}; color: #991B1B; font-weight: 600; }
            </style>
        </head>
        <body>
            <table>
                <tr><th colspan="12" class="title-row">Current Reporting Status Report - ${label}</th></tr>
                <tr>
                    <th>ID</th>
                    <th>Branch</th>
                    <th>Region</th>
                    ...
                    <th class="txt-right">Collection Plan</th>
                    <th class="txt-right">Collection Achievement</th>
                    <th class="txt-right">Achievement %</th>
                    <th class="txt-right">Disbursement Plan (Amt)</th>
                    <th class="txt-right">Disbursement Achieve (Amt)</th>
                    <th class="txt-right">KYC Total</th>
                </tr>
        `;

        const getInt = (val) => parseInt(val) || 0;
        const formatIndian = (num) => num === 0 ? '-' : num.toLocaleString('en-IN');
        ...
        state.rawData.rows.forEach(row => {
            ...
            const t = entry && entry.target ? entry.target : null;
            const a = entry && entry.achievement ? entry.achievement : null;
            ...
            if (t && a) {
                ...
                statusClass = "status-pending-achieve";
            } else if (!t && a) {
                status = "Pending Plan";
                statusClass = "status-pending-plan";
            }

            // Calculations
            const collPlan = t ? (getInt(t.ftod_plan) + getInt(t.nov_25_Slipped_Accounts_Plan) + getInt(t.pnpa_plan)) : 0;
            const collAchieve = a ? (getInt(a.ftod_actual) + getInt(a.nov_25_Slipped_Accounts_Actual) + getInt(a.pnpa_actual)) : 0;
            const achPct = collPlan > 0 ? ((collAchieve / collPlan) * 100).toFixed(2) : '0.00';

            const disbPlan = t ? (getInt(t.disb_igl_amt) + getInt(t.disb_il_amt)) : 0;
            const disbAchieve = a ? (getInt(a.disb_igl_amt) + getInt(a.disb_il_amt)) : 0;

            const kycTotal = a ? (getInt(a.kyc_fig_igl) + getInt(a.kyc_il) + getInt(a.kyc_npa)) : 0;
            ...
            <tr>
                ...
                <td>${district}</td>
                <td>${dm}</td>
                <td class="${statusClass} txt-center">${status}</td>
                <td class="txt-right">${formatIndian(collPlan)}</td>
                <td class="txt-right">${formatIndian(collAchieve)}</td>
                <td class="txt-right" style="font-weight:bold;">${achPct}%</td>
                <td class="txt-right">${formatIndian(disbPlan)}</td>
                <td class="txt-right">${formatIndian(disbAchieve)}</td>
                <td class="txt-right">${formatIndian(kycTotal)}</td>
            </tr>`;
        });
        ...
        const link = document.createElement("a");
        link.setAttribute("href", url);
        link.setAttribute("download", `Current_Status_Report_${label.replace(/ /g, '_')}.xls`);
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        ...
        setLoading(false);
        showToast(`✅ Current Status Report Downloaded`);
    } catch (error) {
        console.error("Current Status Report Error:", error);
        setLoading(false);
        showToast('❌ Failed to generate current status report', true);
    }
}
...
function renderStatRow(label, value, bg, color) {
    return `
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px 16px; background: ${bg}; border-radius: 12px;">
